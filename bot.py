...
# Loyihaning ildiz papkasini Python yo'liga qo'shish (ModuleNotFoundError oldini olish uchun)
...
def setup_cookies():
    ...
    # 1. Secret fayllardan birortasi mavjudligini tekshirish
    found_secret = None
    ...
    if found_secret:
        try:
            size = os.path.getsize(found_secret)
            msg = f"✅ Cookies fayl topildi: {found_secret} ({size} bytes)"
            print(msg)
            logging.info(msg)
        ...
        msg = f"❌ Cookies fayl topilmadi (/etc/secrets/cookies.txt yoki youtube_cookies.txt)"
        print(msg)
        logging.info(msg)
